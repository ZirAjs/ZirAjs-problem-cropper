...
5. 선택을 모두하셨다면 하단의 \[Save Cropped Problems\]버튼을 누르면 저장할 수 있습니다. 저장되는 파일 이름은 `(1.에서 선택한 파일이 있는 폴더의 이름)_(번호).png`로 저장됩니다.

## 기타

시간을 많이 투자할 수 없어서 약 하루 정도 기간에 만든 프로그램이기 때문에 정교하지 않고, 화면 디자인이 아쉬운 부분이 있습니다. 그래도 필요한 곳에 써주시면 감사하겠습니다.
